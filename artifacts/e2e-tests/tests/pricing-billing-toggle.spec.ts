...
test.describe("Pricing Page - Billing Toggle", () => {
  test("toggles between monthly and annual pricing correctly", async ({
    page,
  }) => {
  ...
  test("shows annual period text on paid tiers in annual mode", async ({
    page,
  }) => {
    const pricing = new PricingPage(page);
    await pricing.goto();
    ...
    await pricing.getBillingToggle().click();
    await expect(pricing.getAnnualPriceText("Starter")).not.toBeVisible();
  });

  test("free and enterprise prices remain stable across toggle", async ({
    page,
  }) => {
    const pricing = new PricingPage(page);
    await pricing.goto();
